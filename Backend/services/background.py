...
async def repeatInsert():
    ...
    while True:
        ...
        try:
            await recalculateAllEventImpacts(event_db)
        ...
        try:
            items = await invest_db.uniqueInvestmentItems()
            for item in items:
                ...
                await item_db.insertPredictedPrices(item, item_predictions)
